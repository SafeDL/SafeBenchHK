...
def on_scroll(event):
    """鼠标滚轮缩放地图"""
    global dist, center, road_waypoints
    zoom_factor = 0.9
    if event.button == 'up':
        dist *= zoom_factor
...
def on_motion(event):
    """拖拽地图"""
    global center, last_mouse_pos, road_waypoints
...
def main(config):
    global exit_flag, center, dist, ax, waypoints_sparse, road_waypoints, selected_waypoints_idx
    plt.ion()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', type=str, default="scenario_data/1201-ShaTin12D")
