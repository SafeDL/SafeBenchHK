...
# B2D variant: 39 discrete actions (throttle, steer, brake, reverse)
...
class PIDController(object):
	def __init__(self, K_P=1.0, K_I=0.0, K_D=0.0, n=20):
		...
		self._window = deque([np.float32(0.0) for _ in range(n)], maxlen=n)
		self._max = 0.0
		self._min = 0.0
...
class TCP(nn.Module):
	...
	def __init__(self, config, variant='original'):
		super().__init__()
		self.config = config
		self.variant = variant  # 'original' or 'b2d'
		...
		if self.variant == 'b2d':
			# B2D: 39 discrete actions, decoder_ctrl input = 256 + 39 = 295
			self.num_actions = 39
			self.action_head = nn.Linear(256, self.num_actions)
			self.decoder_ctrl = nn.GRUCell(input_size=256 + self.num_actions, hidden_size=256)
			...
			# Original: Beta distribution, decoder_ctrl input = 256 + 2 + 2 = 260
			dim_out = 2
			...
			self.decoder_ctrl = nn.GRUCell(input_size=256 + dim_out * 2, hidden_size=256)
	...
	def control_pid(self, waypoints, velocity, target):
		...
		if self.variant == 'b2d':
			# B2D: swap x and y axes
			waypoints[:, [0, 1]] = waypoints[:, [1, 0]]
			target[[0, 1]] = target[[1, 0]]
		else:
			# Original: flip y (forward is negative in our waypoints)
			waypoints[:,1] *= -1
			target[1] *= -1
		...
		metadata = {
			'speed': float(speed) if np.ndim(speed) == 0 else float(np.asarray(speed).flatten()[0]),
			'steer': float(steer),
			'throttle': float(throttle),
			'brake': float(brake),
			...
			'desired_speed': float(desired_speed) if np.ndim(desired_speed) == 0 else float(np.asarray(desired_speed).flatten()[0]),
			'angle': float(angle) if np.ndim(angle) == 0 else float(np.asarray(angle).flatten()[0]),
			'angle_last': float(angle_last) if np.ndim(angle_last) == 0 else float(np.asarray(angle_last).flatten()[0]),
			'angle_target': float(angle_target) if np.ndim(angle_target) == 0 else float(np.asarray(angle_target).flatten()[0]),
			'angle_final': float(angle_final) if np.ndim(angle_final) == 0 else float(np.asarray(angle_final).flatten()[0]),
			'delta': float(delta) if np.ndim(delta) == 0 else float(np.asarray(delta).flatten()[0]),
		}

		return steer, throttle, brake, metadata


	def get_action(self, mu_or_action_index, sigma=None):
		if self.variant == 'b2d':
			# B2D: discrete action lookup
			action_index = mu_or_action_index
