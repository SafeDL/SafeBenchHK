        ...
        if len(self._waypoints_queue) < int(self._waypoints_queue.maxlen * 0.5):
            try:
                ...
                if not self._waypoints_queue:
                    # 兜底策略1: 直接用当前车位置的下一个waypoint
                    curr_wp = self._map.get_waypoint(self._vehicle.get_location())
                    next_wps = curr_wp.next(2.0)  # 2.0米之后
                    if next_wps:
                        # 加一个默认 road_option = None
                        self._waypoints_queue.append((next_wps[0], None))
                    else:
                        # 兜底策略2: 如果实在没有next，至少重复当前点
                        self._waypoints_queue.append((curr_wp, None))
