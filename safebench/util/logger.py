...
class Logger:
    ...
    """
    def __init__(self, output_dir=None, output_fname=None, exp_name=None, scenario_category='planning'):
        """
            Initialize a Logger.
        ...
        self.eval_results = []  # 累积所有评估得分（而不是只保留最后一次）
        self.eval_records = {}
    ...
    def print_eval_results(self):
        if not self.eval_results:
            return
        self.log("Evaluation results (latest):")
        latest = self.eval_results[-1]
        for key, value in latest.items():
            self.log(f"\t {key: <25}{value}")
        self.log(f"Total evaluation batches: {len(self.eval_results)}")
    ...
    def dump_tabular(self, x_axis="Epoch", verbose=True, env=None):
        """
            Write all the diagnostics from the current iteration.  Writes both to stdout, and to the output file.
