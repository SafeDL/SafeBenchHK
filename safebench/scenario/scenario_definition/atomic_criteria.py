...
class CollisionTest(Criterion):
    ...
    def _count_collisions(weak_self, event):  # pylint: disable=too-many-return-statements
        """
        更新碰撞计数的回调函数 (已修复：支持列表类型的 other_actor_type)
        """
        self = weak_self()
        if not self:
            return
        ...
        # ==============================================================================
        if self.other_actor_type:
            target_type_id = event.other_actor.type_id

            # --- 分支 A: 如果用户传入的是列表 (例如 ['vehicle', 'walker']) ---
            if isinstance(self.other_actor_type, list):
                is_match = False
                for type_keyword in self.other_actor_type:
                    if type_keyword in target_type_id:
                        is_match = True
                        break
            ...
            # --- 分支 B: 特殊处理 miscellaneous ---
                ...
                if "traffic" not in target_type_id and "static" not in target_type_id:
                    return

            # --- 分支 C: 如果传入的是单个字符串 (例如 'vehicle') ---
            else:
                if self.other_actor_type not in target_type_id:
                    return
        # ==============================================================================
        ...
        # 5. 分类逻辑 (确保只记录有效的交通参与者)
        # 即使通过了上面的过滤，这里再次确认类型以赋予正确的 TrafficEventType
        if 'vehicle' in event.other_actor.type_id:
            ...
            if self.other_actor_type and (
                ...
                return  # 既不是车也不是人，也没显式要求检测静态物，忽略
...
class InRouteTest(Criterion):
    ...
    MAX_ROUTE_PERCENTAGE = 30  # 偏离路线的最大百分比
...
class RouteCompletionTest(Criterion):
    ...
    def update(self):
        ...
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            new_status = Status.FAILURE
        # 逐步遍历路线,self._waypoints,直到找到距离当前车辆位置最近的路线点index
        elif self.test_status == "RUNNING" or self.test_status == "INIT":
            ego_wp_location = [location.x, location.y]
            waypoints_array = np.array([[wp.x, wp.y, wp.z] for wp in self._waypoints])
            distances = np.linalg.norm(waypoints_array[:, :2] - ego_wp_location, axis=1)
            index = np.argmin(distances)
            self._current_index = index
            # print(f"Current index: {self._current_index}")  # debug
            self._percentage_route_completed = 100.0 * float(self._accum_meters[self._current_index]) / float(self._accum_meters[-1])
            # print(f"Percentage route completed: {self._percentage_route_completed}")  # debug
