...
def scenario_parse(config, logger):
    ...
    # list_of_scenario_config: config/scenario_type/xx.json文件,近似可以理解为融合了功能场景和行使路线的配置文件
    list_of_scenario_config = osp.join(ROOT_DIR, config['scenario_type_dir'], config['scenario_type'])
    route_file_formatter = osp.join(ROOT_DIR, config['route_dir'], 'scenario_%02d_routes/scenario_%02d_route_%02d_weather_%02d.xml')
    ...
    # 按照地图将场景配置进行分类
    config_by_map = {}
    for item in data_full:  # data_full存储着所有的route_id,scenario_id,weather_id等信息
        route_file = route_file_formatter % (item['scenario_id'], item['scenario_id'], item['route_id'], item['weather_id'])
        ...
        parsed_config.weather_id = item['weather_id']
...
def filter_valid_spawn_points(spawn_points, current_locations):
    dis_threshold = 8  # 距离阈值
    valid_spawn_points = []  # 用于存储有效的生成点
    for spawn_point in spawn_points:
        valid = True  # 假设当前生成点 spawn_point 是有效的
        for location in current_locations:  # 遍历所有当前的车辆位置 current_locations
            if spawn_point.location.distance(location) < dis_threshold:
                valid = False  # 如果当前生成点与任意一个车辆的位置小于阈值，则将 valid 设为 False
                break
        if valid:
            #  如果 valid 仍为 True，则将当前生成点添加到 valid_spawn_points 列表中。
...
def convert_transform_to_location(transform_vec):
    ...
    """
    # 列表中每一个元素是一个长度为2的tuple,tuple中的第一个元素为carla.location,tuple中的第二个元素为RoadOption
    location_vec = []
